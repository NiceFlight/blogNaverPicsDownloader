...
def Iniwebdriver():
    ...
    options.add_argument('--disable-gpu')  # disable GPU, usually work with --no-headless
...
def Countsofphotos(driver, url):
    try:
        ...
        WebDriverWait(driver, timeout=5).until(EC.presence_of_element_located((By.ID, 'whole-body')))
        # time.sleep(5)
...
def Downloadphotos(driver, imgcounts, url):
    ...
    for img in range(imgcounts):
        try:
            ...
            time.sleep(3)
            soup = BeautifulSoup(driver.page_source, 'html.parser')
            img_url = unquote(soup.find('img', class_='cpv__img cpv__fade').get('data-src'))
        ...
        output_path = os.path.join('images', f'{file_name}.{file_extension}')
        ...
        if download_photo.status_code == 200:
            ...
            print(f'download {file_name} failed!HTTP status code: {download_photo.status_code}')
...
if __name__ == '__main__':
    ...
    if re.fullmatch(pattern, input_url):
        URL = input_url
        print(f'The url you want to download is {URL}')
    else:
